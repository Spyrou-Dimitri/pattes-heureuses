                    ...
                    <x-admin.td>
                        @if(str_starts_with($adoption_pending->animal->avatar, 'public/img/animal/'))
                          ...
                          @else
                            <picture>
                                <source media="(min-width:768px)"
                                        srcset="{{Storage::url('upload_img/animals/variants/128x128/' . $adoption_pending->animal->avatar)}}">
                                <source media="(min-width:576px)"
                                        srcset="{{Storage::url('upload_img/animals/variants/720x720/' . $adoption_pending->animal->avatar)}}">
                                <source media="(max-width:575px)"
                                        srcset="{{Storage::url('upload_img/animals/variants/480x480/' . $adoption_pending->animal->avatar)}}">
                                <img class="img-table"
                                     src="{{Storage::url('upload_img/animals/originals/' . $adoption_pending->animal->avatar)}}"
                                     alt="Photo de {{$adoption_pending->animal->name}}">
                            </picture>
                        @endif
                    </x-admin.td>
